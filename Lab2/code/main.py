...
def sample_examples(docs, max_window_size, n_windows):
    ...
    for _, doc in enumerate(docs):
        ...
        windows.append([window for window in get_windows(doc, window_size)])
    ...
    all_negs = np.random.choice(range(1, len(vocab) + 1), n_negs * len(windows), p=neg_distr)
...
def compute_gradients(pos, negs, target, prodpos, prodnegs):
    ...
    partials_pos = np.outer(-factors_pos, Wt[target,])
    partials_negs = np.outer(factors_negs, Wt[target,])
    ...
    for epoch in range(n_epochs):
        ...
        with tqdm(total=len(windows), unit_scale=True, postfix={'loss': 0.0, 'lr': lr_0},
            ...
            for i, w in enumerate(windows):
                ...
                negs_list = negs.tolist()

                prods = compute_dot_products(pos, negs_list, target)
                ...
                if Wc[pos,].size != 0:
                    Wc[pos,] -= lr * np.array([np.full(d, partial_po) for partial_po in partials_pos])
                Wc[negs,] -= lr * np.array([np.full(d, partial_neg) for partial_neg in partials_negs])
...
if not train:
    ...
    # = = some similarities = = 
    print(my_cos_similarity("waste", "laugh"))
    print(my_cos_similarity("waste", "movie"))
    print(my_cos_similarity("friend", "friends"))
    print(my_cos_similarity("friend", "wanted"))
    print(my_cos_similarity("chocolate", "chomsky"))
