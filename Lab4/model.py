...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        # hs = output.reshape((-1, current_batch_size, self.hidden_dim))
...
class seq2seqAtt(nn.Module):
    ...
    def forward(self, target_h, source_hs):
        ...
        scores = scores.squeeze(
            dim=2)  # (seq,batch,1) -> (seq,batch). dim=2 because we don't want to squeeze the batch dim if batch size = 1
...
class Decoder(nn.Module):
    ...
    def forward(self, input, source_context, h):
        ...
        tilde_h = nn.functional.tanh(self.ff_concat(torch.cat((h, source_context), dim=1)))
...
class seq2seqModel(nn.Module):
    ...
    def __init__(self, vocab_s, source_language, vocab_t_inv, embedding_dim_s, embedding_dim_t,
        ...
        self.encoder = Encoder(self.max_source_idx + 1, self.embedding_dim_s, self.hidden_dim_s, self.padding_token).to(
            self.device)
        self.decoder = Decoder(self.max_target_idx + 1, self.embedding_dim_t, self.hidden_dim_t, self.padding_token).to(
            self.device)
    ...
    def forward(self, input, max_size, is_prod):
        ...
        if is_prod:
            input = input.unsqueeze(
                1)  # (seq) -> (seq,1) 1D input <=> we receive just one sentence as input (predict/production mode)
    ...
    def load(cls, path_to_file):
        attrs = torch.load(path_to_file, map_location=lambda storage,
                                                             loc: storage)  # allows loading on CPU a model trained on GPU, see https://discuss.pytorch.org/t/on-a-cpu-device-how-to-load-checkpoint-saved-on-gpu-device/349/6
