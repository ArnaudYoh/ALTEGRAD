        ...
        for epoch in range(n_epochs):
            ...
            with tqdm(total=136521, unit_scale=True, postfix={'loss': 0.0}, desc="Epoch %i/%i" % (epoch + 1, n_epochs)) \
                ...
                for i, (source, target) in enumerate(train_loader):
                    ...
                    optimizer.zero_grad()
                    loss = criteron(pred[:, 1:].flatten(end_dim=1), target.flatten())
                    loss.backward()
                    optimizer.step()
...
class MultiHeadAttention(Module):
    ...
    def __init__(self, N_heads, dk, dv, dmodel, device):
        ...
        self.Wq = torch.nn.Parameter(torch.randn((N_heads, dmodel, dk), requires_grad=True, device=device))
        self.Wk = torch.nn.Parameter(torch.randn((N_heads, dmodel, dk), requires_grad=True, device=device))
        self.Wv = torch.nn.Parameter(torch.randn((N_heads, dmodel, dv), requires_grad=True, device=device))
        self.Wo = torch.nn.Parameter(torch.randn((dmodel, dmodel), requires_grad=True, device=device))
